...
const Accordion = ({ accordionItem }) => {
  ...
  return (
    <li className="border-b border-slate overflow-hidden mb-2 md:mb-4">
      ...
      >
        <span
          className="bg-animate bg-animate-electric-lime cursor-pointer md:hover:text-not-dark-blue"
        ...
        <button
          onClick={() => setExpanded(expanded ? false : true)}
          className="absolute right-0 top-1/2 h-10 w-10 transform -translate-y-1/2 text-cream border border-slate rounded-full bg-animate bg-animate-electric-lime cursor-pointer md:hover:border-not-dark-blue md:hover:text-not-dark-blue"
        >
          <svg className={`absolute inset-0 h-10 w-10 transform origin-center transition-transform ease-out duration-500 ${expanded ? 'rotate-180' : ''}`} width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path className="fill-current" fillRule="evenodd" clipRule="evenodd" d="M26 19.5C26 19.7761 25.6162 20 25.1429 20L14.8571 20C14.3838 20 14 19.7761 14 19.5C14 19.2239 14.3838 19 14.8571 19L25.1429 19C25.6162 19 26 19.2239 26 19.5Z" />
          </svg>
