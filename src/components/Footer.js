            ...
            <ul className="flex flex-wrap justify-center space-x-3">
              {settings.footer.socialMediaLinks.map((data, index) => {
                return <li key={`content_item_${index}`}>
                  <a className="group bg-animate bg-animate-voltage text-electric-lime block rounded-full overflow-hidden md:hover:text-not-dark-blue" href={data.url} target="_blank">
                    {data.type === 'linkedin' &&
                      <svg className="bg-inner-animate relative z-10 rounded-full border border-electric-lime w-9 h-9 group-hover:border-voltage sm:w-12 sm:h-12" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
