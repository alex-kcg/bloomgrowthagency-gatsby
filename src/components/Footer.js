        ...
        <ul className="flex flex-wrap justify-center space-x-3 mb-2 sm:mb-0">
          {settings.footer.socialMediaLinks.map((data, index) => {
            return <li key={`content_item_${index}`}>
              <a className="group button button-violet text-concrete block rounded-full overflow-hidden hover:text-ebony" href={data.url} target="_blank">
                {data.type == 'linkedin' &&
                  <svg  className="relative z-10 rounded-full border border-lime w-9 h-9 group-hover:border-violet sm:w-12 sm:h-12" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
