            ...
            <ul className="flex flex-wrap justify-center space-x-3">
              {settings.footer.socialMediaLinks.map((data, index) => {
                return <li key={`content_item_${index}`}>
                  <a className="group bg-animate bg-animate-voltage text-electric-lime block rounded-full overflow-hidden hover:text-not-dark-blue" href={data.url} target="_blank">
                    {data.type === 'linkedin' &&
