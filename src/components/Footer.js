...
const Footer = class extends React.Component {
  render() {
    return (
      <footer className="footer sticky z-50 bottom-0 bg-ebony text-gray text-sm w-full mt-auto">
          ...
          <ul className="flex flex-wrap justify-center space-x-3 mb-2 sm:mb-0">
            {settings.footer.socialMediaLinks.map((data, index) => {
              return <li key={`content_item_${index}`}>
                <a className="group button button-voltage text-electric-lime block rounded-full overflow-hidden hover:text-ebony" href={data.url} target="_blank">
                  {data.type == 'linkedin' &&
